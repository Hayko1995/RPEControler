...
class DatabaseService {
  ...
  Future<void> insertLightSensors(LightSensors breed) async {
  ...
  Future<List<LightSensors>> lightSensors(LightSensors lightSensors) async {
    // Get a reference to the database.
    final db = await _databaseService.database;

    // Query the table for all the Breeds.
    final List<Map<String, dynamic>> maps = await db.query(AppConstants.light);

    // Convert the List<Map<String, dynamic> into a List<Breed>.
    return List.generate(
        maps.length, (index) => LightSensors.fromMap(maps[index]));
  }

  Future<LightSensors> lightSensor(int id) async {
    final db = await _databaseService.database;
    final List<Map<String, dynamic>> maps =
        await db.query(AppConstants.light, where: 'id = ?', whereArgs: [id]);
    return LightSensors.fromMap(maps[0]);
  }

  // A method that updates a breed data from the breeds table.
  Future<void> updateLightSensor(LightSensors breed) async {
    // Get a reference to the database.
    final db = await _databaseService.database;
  ...
  Future<void> deleteLightSensor(int id) async {
    // Get a reference to the database.
    final db = await _databaseService.database;
